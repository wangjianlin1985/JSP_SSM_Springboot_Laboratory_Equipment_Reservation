...
public class ClassInfoController extends BaseController {
	...
	public void list(String classNo,String specialName,String className,String bornDate,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<ClassInfo> classInfoList = classInfoService.queryClassInfo(classNo, specialName, className, bornDate, page);
	    /*计算总的页数和总的记录数*/
	    classInfoService.queryTotalPageAndRecordNumber(classNo, specialName, className, bornDate);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String classNo,String specialName,String className,String bornDate,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (classNo == null) classNo = "";
		if (specialName == null) specialName = "";
		if (className == null) className = "";
		if (bornDate == null) bornDate = "";
		List<ClassInfo> classInfoList = classInfoService.queryClassInfo(classNo, specialName, className, bornDate, currentPage);
	    /*计算总的页数和总的记录数*/
	    classInfoService.queryTotalPageAndRecordNumber(classNo, specialName, className, bornDate);
	    /*获取到总的页码数目*/
	    int totalPage = classInfoService.getTotalPage();
	...
	public void OutToExcel(String classNo,String specialName,String className,String bornDate, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(classNo == null) classNo = "";
        if(specialName == null) specialName = "";
        if(className == null) className = "";
        if(bornDate == null) bornDate = "";
        List<ClassInfo> classInfoList = classInfoService.queryClassInfo(classNo,specialName,className,bornDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "ClassInfo信息记录"; 
        String[] headers = { "班级编号","所属专业","班级名称","成立日期","班主任"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<classInfoList.size();i++) {
        	ClassInfo classInfo = classInfoList.get(i); 
        	dataset.add(new String[]{classInfo.getClassNo(),classInfo.getSpecialName(),classInfo.getClassName(),classInfo.getBornDate(),classInfo.getMainTeacher()});
        }
        /*

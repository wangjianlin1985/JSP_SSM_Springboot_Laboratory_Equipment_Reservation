...
public class TeacherController extends BaseController {
	...
	public void list(String telephone,String teacherNo,String teacherName,String inDate,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<Teacher> teacherList = teacherService.queryTeacher(telephone, teacherNo, teacherName, inDate, page);
	    /*计算总的页数和总的记录数*/
	    teacherService.queryTotalPageAndRecordNumber(telephone, teacherNo, teacherName, inDate);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String telephone,String teacherNo,String teacherName,String inDate,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (telephone == null) telephone = "";
		if (teacherNo == null) teacherNo = "";
		if (teacherName == null) teacherName = "";
		if (inDate == null) inDate = "";
		List<Teacher> teacherList = teacherService.queryTeacher(telephone, teacherNo, teacherName, inDate, currentPage);
	    /*计算总的页数和总的记录数*/
	    teacherService.queryTotalPageAndRecordNumber(telephone, teacherNo, teacherName, inDate);
	    /*获取到总的页码数目*/
	    int totalPage = teacherService.getTotalPage();
	...
	public void OutToExcel(String telephone,String teacherNo,String teacherName,String inDate, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(telephone == null) telephone = "";
        if(teacherNo == null) teacherNo = "";
        if(teacherName == null) teacherName = "";
        if(inDate == null) inDate = "";
        List<Teacher> teacherList = teacherService.queryTeacher(telephone,teacherNo,teacherName,inDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Teacher信息记录"; 
        String[] headers = { "教师工号","教师姓名","教师性别","教师照片","入职日期","联系电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<teacherList.size();i++) {
        	Teacher teacher = teacherList.get(i); 
        	dataset.add(new String[]{teacher.getTeacherNo(),teacher.getTeacherName(),teacher.getTeacherSex(),teacher.getTeacherPhoto(),teacher.getInDate(),teacher.getTelephone()});
        }
        /*
